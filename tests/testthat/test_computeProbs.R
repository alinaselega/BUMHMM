...
covFile = matrix(c(100,50,60,50,70,80,60,50,60,50,80,80,70,70,60,70,100,80,80,70),
                 nrow = 5, ncol = 4)
...
LDR_C <- stabiliseVariance(covFile, dorFile, nuclSelection$analysedC,
                            nuclSelection$analysedCT, 2, 2)$LDR_C
...
test_that("function computes probabilities correctly", {
    ...
    expect_equal(computeProbs(LDR_C, LDR_CT, 2, 2, '+', nuclPosition,
                            nuclSelection$analysedC,
                            nuclSelection$analysedCT, stretches)[3,2], 1,
               tolerance=1e-7)
})
