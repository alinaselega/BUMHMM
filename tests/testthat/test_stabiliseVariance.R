...
test_that("function throws error when all quantiles are 0", {
    expect_error(stabiliseVariance(covFile, dorFile, nuclSelection$analysedC,
                                 nuclSelection$analysedCT, 2, 2),
...
LDR_CT <- stabiliseVariance(covFile, dorFile, nuclSelection$analysedC,
                               nuclSelection$analysedCT, 2, 2)$LDR_CT

test_that("function rescales LDRs correctly", {
    expect_equal(stabiliseVariance(covFile, dorFile, nuclSelection$analysedC,
                                 nuclSelection$analysedCT, 2, 2)$LDR_C[1,1],
               -8.238417, tolerance=1e-7)
    expect_equal(LDR_CT[1,2], 15.9536421, tolerance=1e-7)
})
...
test_that("function only rescales LDRs from selected positions", {
    expect_equal(all(is.na(LDR_CT[notSelected[[1]], indexT[1,1]])), TRUE)
    expect_equal(all(is.na(LDR_CT[notSelected[[2]], indexT[2,1]])), TRUE)
    expect_equal(all(is.na(LDR_CT[notSelected[[3]], indexT[3,1]])), TRUE)
    expect_equal(all(is.na(LDR_CT[notSelected[[4]], indexT[4,1]])), TRUE)
})
